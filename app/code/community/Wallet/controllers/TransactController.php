...
class Wallet_TransactController extends Mage_Core_Controller_Front_Action 
{
	...
	$walletConfig = Mage::getStoreConfig('payment/wallet');
        ...
        if ($this->_validateResponse()) {
			...
			$total_amount = (int)$order2->getGrandTotal();  //// total amount from database for an order ////			
			...
			if( ($postdata['amount'] == (int)$order2->getGrandTotal()) AND ($session->getLastRealOrderId() == $postdata['orderid'])) {

				error_log('order macthed');
			    // calculate checksum //
				$action_wallet = "gettxnstatus"; // fixed value
				$algo_wallet = 'sha256';
				
                $MerchantId =  $walletConfig['merchant_id'] ; // merchant ID //
				$WorkingKey	=  $walletConfig['secret_key'] ; // merchant key ///			
				$checksum_string_wallet = "'{$action_wallet}''{$MerchantId}''{$postdata['orderid']}'";
				...
				if(($checksum_check_api == $outputXmlObject->checksum) && ((int)$outputXmlObject->amount == $postdata['amount']) && ($postdata['amount'] == $total_amount) ){	
