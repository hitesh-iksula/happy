...
class Wallet_Model_Transact extends Mage_Payment_Model_Method_Abstract 
{
    ...
    protected $_formBlockType = 'wallet/form';
    ...
    {
       return 'https://www.mobikwik.com/views/proceedtowalletpayment.jsp';	   
    }
    ...
    /*public function capture(Varien_Object $payment, $amount) 
    {
        ...
        $api = Mage::getModel('wallet/api_update');
        $api->send($order->getIncrementId(), Wallet_Model_Api_Update::$STATUS_SETTLED, 'payment captured');        
        if ($api->getResponseCode() == 196) {
            Mage::throwException('Online Capture failed. Wallet Update Api responded Response Code: '.$api->getResponseCode() . ' Message: ' . $api->getResponseDescription());
            Mage::log('Wallet update api failure: orderId: '.$order->getIncrementId() . ', statuscode: ' . $api->getResponseCode(). ', statusmessage: ' . $api->getResponseDescription());
        }
    }
    ...
    public function cancel(Varien_Object $payment) 
    {
        $order = $payment->getOrder();
        $api = Mage::getModel('wallet/api_update');
        $api->send($order->getIncrementId(), Wallet_Model_Api_Update::$STATUS_CANCELLED, 'order cancelled');        
        if (!in_array($api->getResponseCode(), Wallet_Model_Api_Update::$CANCELLED_RESP_CODES)) {
            Mage::throwException('Request could not be completed since Wallet Update Api responded with update failure. Response Code: '.$api->getResponseCode());
            Mage::log('Wallet update api failure: orderId: '.$order->getIncrementId() . ', statuscode: ' . $api->getResponseCode(). ', statusmessage: ' . $api->getResponseDescription());
        }
    }
    ...
    public function void(Varien_Object $payment) {
        $order = $payment->getOrder();
        $api = Mage::getModel('wallet/api_update');
        $api->send($order->getIncrementId(), Wallet_Model_Api_Update::$STATUS_CANCELLED, 'order void');        
        if (!in_array($api->getResponseCode(), Wallet_Model_Api_Update::$CANCELLED_RESP_CODES)) {
            Mage::throwException('Request could not be completed since Wallet Update Api responded with update failure. Response Code: '.$api->getResponseCode());
            Mage::log('Wallet update api failure: orderId: '.$order->getIncrementId() . ', statuscode: ' . $api->getResponseCode(). ', statusmessage: ' . $api->getResponseDescription());
        }
