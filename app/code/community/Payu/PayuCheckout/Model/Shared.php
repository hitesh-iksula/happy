...
class Payu_PayuCheckout_Model_Shared extends Mage_Payment_Model_Method_Abstract
{
	...
	public function getFormFields()
	{
		...
		$payuPaymentMethod        = $order->getPayment()->getPayuPaymentMethod();
		$payuMethodOption         = $order->getPayment()->getPayuMethodOption();

		$coFields['pg']           =  $payuPaymentMethod;
		$coFields['bankcode']     =  $payuMethodOption;
...
else
{
 ...
 $coFields['productinfo'].'|'.$coFields['firstname'].'|'.$coFields['email'].'|||||||||||'.$salt));
		...
		/*echo '<pre>';
		echo '$payuPaymentMethod = ' . $payuPaymentMethod;
		echo '$payuMethodOption = ' . $payuMethodOption;
		print_r($coFields);
		exit;*/
		   ...
		   if($response['status']=='success')
			{
				...
				if($sentHashString==$responseHashString)
				{
					...
					if($response['PG_TYPE'] == 'HDFCPG') {
						$order->getPayment()->setMethod('ccavenue')->save();
					}
