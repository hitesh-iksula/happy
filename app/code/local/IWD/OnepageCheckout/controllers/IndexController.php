...
class IWD_OnepageCheckout_IndexController extends Mage_Checkout_Controller_Action
{
    ...
    {
    	$layout	= $this->_getUpdatedLayout();
        if(!$layout->getBlock('cart_sidebar')) {
            return '';
        }
        return $layout->getBlock('cart_sidebar')->toHtml();
    }
...
/*--------------------------------*/
    ...
    public function updateCheckoutAction()
    {
        ...
        /* one day delivery is not available check */
        $postcode = $bill_data['postcode'];
        $pincodeModel = Mage::getModel('pincode/pincode');
        $collection = $pincodeModel->getCollection()->addFieldToFilter('pincode', $postcode)->getFirstItem();

        $result['pincode_info'] = $collection->getPincode();

        if(!$collection->getPincode() && $ship_method == 'flatrate_flatrate') {
            $ship_method = 'tablerate_bestway';
        }
        /* one day delivery is not available check */
...
///////////////
        ...
        $result['update_section']['cart-sidebar'] = $this->_getSidecartHtml();

        /*********** DISCOUNT CODES **********/
    	if ($couponChanged) {
            if ($couponData['code'] == $quote->getCouponCode()) {
                if($couponCancelled == false) {
        ...
        $formattedAddress = Mage::helper('explore')->getFormattedHtmlFromArray($bill_data);
        $shipping = $this->getOnepagecheckout()->getQuote()->getShippingAddress();
        if($shipping) {
            $formattedAddress = $shipping->getFormated('raw');
        }
        $result['update_section']['shipping_info'] = $formattedAddress;
        $result['update_section']['shipping_info_cod'] = $formattedAddress;

        $result['shpm'] = $ship_method;
    ...
    public function saveOrderAction()
    {
        ...
        try {
            ...
            $this->getOnepagecheckout()->getQuote()->setOrderSource($this->getRequest()->getPost('order_source'));
