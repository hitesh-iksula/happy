 ...
 */
class Iksula_Override_Model_Order extends Mage_Sales_Model_Order
{
    /**
    ...
    public function sendNewOrderEmail()
    {
        ...
        $delivery_rate = $this->getData('shipping_method');

        if($delivery_rate == 'tablerate_bestway'){ 
            $delivery_date = '3 - 4 working days';
        }
        else{
            $nextshipmentday = 'Monday,';
                if(($myday1 == 'Fri') && ($currentTime >= 16) ){
                    $delivery_date = Date('d/m/Y', strtotime("+3 days"));
                    $delivery_day = $nextshipmentday;
                } 
                elseif($myday1 == 'Sat'){
                    $delivery_date = Date('d/m/Y', strtotime("+2 days")); 
                    $delivery_day = $nextshipmentday; 
                }
                elseif(($myday1 == 'Sun') && ($currentTime <= 16) ){
                    $delivery_date = Date('d/m/Y', strtotime("+1 days")); 
                    $delivery_day = $nextshipmentday; 
                }
                elseif(($myday1) && ($currentTime >= 16)  && ($myday1 != 'Fri') && ($myday1 != 'Sat')){
                    $delivery_date = Date('d/m/Y', strtotime("+2 days")); 
                    $delivery_day = $nextday1; 
                }
                else{
                    $delivery_date = Date('d/m/Y', strtotime("+1 days")); 
