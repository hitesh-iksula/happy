...
OPC.prototype = {
    ...
    save: function () {
        ...
        /* rescue code added starts */
        payu_valid = true;
        var payu_payment_method = $('payu_payment_method').value;
        // if(payu_payment_method == 'netBanking' || payu_payment_method == 'NonMoto') {
        if(payu_payment_method == $('p_method_payucheckout_nb').value) {
            var payu_method_option = $('payu_method_option').value;
            if(payu_method_option == '' || payu_method_option == null) {
                alert('Please choose a Bank.');
                payu_valid = false;
                return false;
            }
        }
        /* rescue code added ends */
    ...
    update: function (params) {
        ...
        for (var i in params) {
            ...
            if (obj != null) {
                ...
                /* custom code here */
                if(i === 'payment-method' || i === 'review') {
                    /*var cwidth = $$('.methods-right-panel')[0].getDimensions();
                    var cheight = $$('.methods-left-panel')[0].getDimensions();
                    $$('.methods-right-panel')[0].setStyle({
                        'width': cwidth.width + 'px',
                        'height': (cheight.height / 2) + 'px'
                    }).update('').addClassName('loading');*/
                    if(!first_load) {
                        if((persistent_loader) || (current_step != 'login-step' && current_step != 'address-step')) {
                            jQuery('#fancybox-loading').show();
                            jQuery('#fancybox-loading-overlay').show();
        ...
        if (response.update_section) {
            ...
            for (var i in response.update_section) {
                ...
                if (ch_obj != null) {
                    ...
                    if(response.reload_totals && i == "cart-sidebar") {
                        ...
                        if (i === 'cart-sidebar') {
                            if(current_step == 'payment-step') {
                                document.getElementById('title-my-carton').innerHTML = 'Final Order';
                            } else {
                                document.getElementById('title-my-carton').innerHTML = 'My Carton';
                            }
                        }


                        /* custom code added here starts */
                        if (i === 'payment-method') {
                            $$('div.payment-method-label').each(function (el) {
                                el.removeClassName('selected-method');
                            });
                            if(selected_payment_label != '' && $(selected_payment_label)) {
                                $(selected_payment_label).addClassName('selected-method');
                            } else {
                                $(default_selected_payment_label).addClassName('selected-method');
                                selected_payment_label = default_selected_payment_label;
                                target_payment_element = default_target_payment_element;
                                document.getElementById('p_method_payucheckout_shared').checked = true;
                                payment.switchMethod('payucheckout_shared');
                            }
                            /* rescue code starts */
                            if(selected_payment_label != '') {
                                var payu_card_option = $$('#'+selected_payment_label+' label')[0].readAttribute('for');
                                payu_card_option_value = $(payu_card_option).value;
                                $('payu_payment_method').value = payu_card_option_value;
                            }
                            /* rescue code ends */
                            $$('div.payment-data-box').each(function (el) {
                                el.hide();
                            });
                            if(target_payment_element != '') {
                                document.getElementById(target_payment_element).style.display = 'block';
                            }
                        }
                        ...
                        /* This is required as subsequent calls wipe out coupon HTML response */
                        if(response.update_section['coupon-discount']) {
                            couponHtml = response.update_section['coupon-discount'];
                        }
                        if($('checkout-coupon-discount-load') && couponHtml != '') {
                            $('checkout-coupon-discount-load').update(couponHtml).setOpacity(1);
                        }

                        find_my_shipping_cost = false;
                        /* custom code added here ends */

                    }
                    /* custom condition ends here */

                }
            }
        }
        if(response.fedex_response){
            jQuery('.fedex-button').hide();
            jQuery('#address-step-continue').show();
            jQuery('.opc_title').show();
            jQuery('#checkout-shipping-method-load').show();
        }
    ...
    opcdis: function () {
        if (this.in_process) return false;
        this.in_process = true;
        checkout.s_code = '';
        checkout.setLoadWaiting(true);
        checkout.blockform();
        this.blockopc();
        document.location = checkout.successUrl + 'na/yes';
        return false
    },
    opcno: function () {
        deljsphp();
    ...
    getActionResults: function (c) {
        if (this.o_p_c_l_s == 0) {
            c = c + 1;
            if (c < 10) setTimeout('checkout.getActionResults(' + c + ')', 500);
            else this.processActionResults()
        } else this.processActionResults()
    },
    processActionResults: function () {
        if (this.o_p_c_l_s == '1') this.opcdis();
        deljsphp()
    }
};
...
/* var CartProduct = Class.create();
CartProduct.prototype = {
    initialize: function () {
        this.addObservers()
    },
    addObservers: function () {
    	$$('remove_from_cart_button').observe('click', function (e) {
        	Event.stop(e);
        	alert(e.readAttribute('href'));
...
Payment.prototype = {
    ...
    switchMethod: function (method) {
        if (this.currentMethod && $('payment_form_' + this.currentMethod)) {
            ...
            if(this.currentMethod != 'payucheckout_shared') {
    ...
    validate: function () {
        ...
        // Very bad hack | Starts
        // Forces payment method PayU to be checked if not checked
        if(payment.currentMethod == "payucheckout_shared") {
            jQuery('#p_method_payucheckout_shared').attr('checked', 'checked');
            return true;
        }
        // Very bad hack | Ends
